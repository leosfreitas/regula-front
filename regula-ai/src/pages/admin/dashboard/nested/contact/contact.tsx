    ...
    <div className="flex flex-col items-center justify-start min-h-[80vh] bg-gray-100 px-4 w-full py-8">
      <div className="w-full max-w-7xl bg-white rounded-lg shadow-lg p-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800">Administração de Contatos</h1>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <p className="text-gray-500 text-xl mb-4">Erro ao carregar contatos. Tente novamente.</p>
          </div>
        ) : filteredContacts.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-gray-500 text-xl mb-4">Nenhum contato encontrado.</p>
          </div>
        ) : (
        ...
        <DialogContent className="max-w-2xl bg-white rounded-lg shadow-lg fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 p-6 z-50 border border-gray-200">
          <DialogHeader>
            <DialogTitle className="flex justify-between items-center text-xl font-bold">
              ...
              {selectedContact && (
                <span className={`px-3 py-2 rounded-full text-sm font-medium ${getStatusColor(selectedContact.status)}`}>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="text-lg font-medium">{selectedContact.nome}</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-lg">{selectedContact.empresa || "-"}</p>
                </div>
                
                <div>
                  <p className="text-sm text-gray-500 mb-1">Telefone</p>
                  <p className="text-lg">{selectedContact.telefone || "-"}</p>
                </div>
              </div>

              <div className="mb-6">
                <p className="text-sm text-gray-500 mb-1">Mensagem</p>
                <p className="p-4 bg-gray-50 rounded-lg border border-gray-200 whitespace-pre-wrap max-h-60 overflow-auto">
